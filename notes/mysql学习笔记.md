...
- 存储引擎是对数据的一种存储机制，如何存储数据以及如何查询提取出数据
...
- Buffer Poll 是 MySQL 服务器启动时向操作系统申请的一片连续的内存空间
- 每个缓存页对应一个含控制信息的控制块
- 空闲链表，即 free 链表为空闲控制块组成的链表
- flush 链表为存储脏页的链表，即修改过的但还未存磁盘中的缓存页对应的控制块组成的链表
- LRU (least recently used) 链表，当空闲缓存页不够时按照最近最少使用的原则淘汰缓存页的链表
- 预读和全表扫描可能造成缓存页的命中率降低，因此将 LRU 链表划分为 young 区（热数据）和 old 区域（冷数据）

### 刷新脏数据到磁盘
- 后台有专门隔一段时间将脏页刷新到磁盘的线程，不影响用户的正常请求
- 刷新途径有两种
   - 从 LRU 链表中的冷数据中刷新一部分到磁盘，即 BUF_FLUSH_LRU
...
在某些隔离级别下，例如读已提交（Read Committed）和可重复读（Repeatable Read），可以解决不可重复读问题。

在读已提交隔离级别下，事务只能读取到已经提交的数据，因此不会遇到不可重复读的问题。
但是，在同一个事务中多次读取同一数据可能会得到不同的结果，因为其他事务可能会在事务执行期间对数据进行修改。

在可重复读隔离级别下，事务在开始时创建一个一致性视图，保持事务开始时数据库的快照状态。
这样，在同一个事务内多次读取同一数据时，会得到一致的结果，避免了不可重复读的问题。

## 幻读
"幻读"（Phantom Read）是数据库并发访问中的一种问题
指在一个事务中多次执行**同一个查询**，但得到的结果集却不同。
...
幻读问题主要是由于并发事务之间的数据插入、修改或删除引起的。
在某些隔离级别下，例如可重复读（Repeatable Read）和串行化（Serializable），可以解决幻读问题。
...
幻读主要是针对数据的新增操作，即一个事务在查询数据之后，另一个事务对数据进行了新增，导致查询结果中出现了新增的数据。
...
以下是导致可重复读隔离级别下可能发生幻读的几种情况：

1. 插入操作：在可重复读隔离级别下，事务中的查询会创建一个一致性视图，保持事务开始时数据库的快照状态。但如果在事务执行期间发生了其他事务的插入操作，新增的数据行在下一次相同查询时会被包含在结果集中，导致幻读的发生。

2. 删除操作：类似地，可重复读隔离级别下的一致性视图不会包含其他事务删除的数据行。如果在事务执行期间发生了其他事务的删除操作，下一次相同查询可能会包含之前被删除的数据行，引发幻读。

3. 范围查询锁定：可重复读隔离级别下，事务可以对查询结果集中的数据行进行锁定，以防止其他事务对这些数据行进行修改。然而，如果其他事务在事务执行期间插入或删除了符合查询条件的数据行，那么在同一个事务内再次执行相同查询时，就会发现新增或删除的数据行，导致幻读。
...
- undo log 记录的是事务开始前的数据状态，为了回滚
- 每发生一次增删改操作，就生成一个事务id，记录对应的 undo 日志
